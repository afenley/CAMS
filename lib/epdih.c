...
/* the following function computes the proper dihedral energy
 */
float pdihedralenergy(struct topology *mol)
{
	...
	energy = 0.0;
	for(i=0;i<mol->numpdihedrals;i++)
	{
		atomi = mol->pdihedrals[i].atomi;
		atomj = mol->pdihedrals[i].atomj;
		atomk = mol->pdihedrals[i].atomk;
		atoml = mol->pdihedrals[i].atoml;
		...
		energy = energy + mol->pdihedrals[i].fc*(1.0+cos(mol->pdihedrals[i].mult*theta*sign - mol->pdihedrals[i].phi));
	}
	return energy;
}
