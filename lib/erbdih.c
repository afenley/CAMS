...
/* the following function computes the rb dihedral energy
 */
float rbdihedralenergy(struct topology *mol)
{
	...
	energy = 0.0;
	for(i=0;i<mol->numrbdihedrals;i++)
	{
		atomi = mol->rbdihedrals[i].atomi;
		atomj = mol->rbdihedrals[i].atomj;
		atomk = mol->rbdihedrals[i].atomk;
		atoml = mol->rbdihedrals[i].atoml;
		...
		energy = energy + mol->rbdihedrals[i].k0;
		energy = energy + mol->rbdihedrals[i].k1*(cos(theta*sign));
		energy = energy + mol->rbdihedrals[i].k2*(-cos(2.0*theta*sign));
		energy = energy + mol->rbdihedrals[i].k3*(+cos(3.0*theta*sign));
		energy = energy + mol->rbdihedrals[i].k4*(-cos(4.0*theta*sign));

		//energy = energy + mol->rbdihedrals[i].C0;// * pow(cos(theta*sign - M_PI),0);
		//energy = energy + mol->rbdihedrals[i].C1 * pow(cos(theta*sign - M_PI),1);
		//energy = energy + mol->rbdihedrals[i].C2 * pow(cos(theta*sign - M_PI),2);
		//energy = energy + mol->rbdihedrals[i].C3 * pow(cos(theta*sign - M_PI),3);
		//energy = energy + mol->rbdihedrals[i].C4 * pow(cos(theta*sign - M_PI),4);
		//energy = energy + mol->rbdihedrals[i].C5 * pow(cos(theta*sign - M_PI),5);
	}
	return energy;
}
